...
public class server_unsafe {
    ...
    private static final MessageHandlerBroken messageHandler = new MessageHandlerBroken();
                    ...
                    if (message.startsWith("/")) {
                        switch (message.split(" ")[0]) {
                            ...
                            case "/Eprofile":
                                ...
                                String profileElementChange = reader.readLine(); // Read profile element to edit
                                String actualChange = reader.readLine(); // Read new value for the profile element

                                // Check if the target user exists
                                if (clientProfiles.containsKey(clientChange)) {
                                    Thread.sleep(3000); // Sleep 3 seconds for demo's sake
                                    // Get the target user's profile
                                    HashMap<String, String> targetProfile = clientProfiles.get(clientChange);
                                    // Update the specified profile element with the new value
                                    targetProfile.put(profileElementChange, actualChange);
                                    writer.println(
                                            "Profile element '" + profileElementChange + "' successfully edited for "
                                                    + clientChange);
                                } else {
        ...
        private void handleMessage(String message, PrintWriter writer) {
            ...
            if (parts.length >= 3) {
                ...
                if (targetWriter != null) {
                    ...
                    writer.println(
                            "Sent: " + privateMessage + " to " + targetUser + " on the logs since they are offline");
                } else {
